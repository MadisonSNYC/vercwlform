...
>(({ index, className, ...props }, ref) => (
  <Slot
    ...
    index={index}
    className={cn(
      "relative flex h-9 w-9 items-center justify-center border-y border-r border-input text-sm shadow-sm transition-all first:rounded-l-md first:border-l last:rounded-r-md focus:z-10 group-data-[focus]:border-ring group-data-[hover]:border-accent-foreground",
      className,
    )}
    {...props}
  />
